...
class upnp:
    ...
    def initSockets(self, ip, port, iface):
        ...
        try:
            ...
            try:
                ...
                print "WARNING: Failed to bind %s:%d: %s", \
                    (self.ip, self.port, e)
            try:
    ...
    def send(self, data, socket):
        ...
        try:
            ...
            print "SendTo method failed for %s:%d : %s" \
                % (self.ip, self.port, e)
            return False
    ...
    def parseURL(self, url):
        ...
        try:
            ...
            #If '://' is not in the url, then it's not a full URL,
            # so assume that it's just a relative path
            page = url

        return (host, page)

    #Pull the name of the device type from a device type string
    #The device type string looks like:
    ...
    #The service type string looks like:
    ...
    def parseSSDPInfo(self, data, showUniq, verbose):
        ...
        if messageType is not False:
            ...
            #   2. We've already seen this host,
            #       but the uniq hosts setting is disabled
            ...
            if (hostFound and not self.UNIQ) or not hostFound:
                #Get the new host's index number and create
                #   an entry in ENUM_HOSTS
                ...
                #   through this host's data structure
    ...
    #Send SOAP request
    def sendSOAP(self, hostName, serviceType,
                 controlURL, actionName, actionArguments):
        argList = ''
        soapResponse = ''
        ...
        #Check if a port number was specified in the host name;
        #   default is port 80
        if ':' in hostName:
            ...
            try:
                ...
                print 'Invalid port specified for host connection:', \
                    hostName[1]
                return False
        else:
            host = hostName
        ...
        #Create a string containing all of the
        #   SOAP action's arguments and values
        ...
        soapBody = '<?xml version="1.0"?>\n'\
            ...
            '\t</m:%s>\n'\
            '</SOAP-ENV:Body>\n'\
            '</SOAP-ENV:Envelope>' % (actionName,
                                      serviceType, argList, actionName)
        ...
        try:
            ...
            if not header.upper().startswith('HTTP/1.') and ' 200 ' in\
                    header.split('\r\n')[0]:
                print 'SOAP request failed with error code:',\
                    header.split('\r\n')[0].split(' ', 1)[1]
    ...
    def showCompleteHostInfo(self, index, fp):
        ...
        try:
            ...
            if hostInfo['dataComplete'] is False:
                print "Cannot show all host info because I don't have it all'\
                     ' yet. Try running 'host info %d' first...\n" % index
            ...
            for deviceName, deviceStruct in hostInfo['deviceList'].iteritems():
                ...
                for serviceName, serviceStruct in \
                        deviceStruct['services'].iteritems():
                    fp.write('\t\tService Name: %s\n' % serviceName)
                    for key in serviceKeys:
                        fp.write('\t\t\t%s: %s\n' % (key, serviceStruct[key]))
                    fp.write('\t\t\tServiceActions:\n')
                    for actionName, actionStruct in \
                            serviceStruct['actions'].iteritems():
                        fp.write('\t\t\t\t%s\n' % actionName)
                        for argName, argStruct in \
                                actionStruct['arguments'].iteritems():
                            fp.write('\t\t\t\t\t%s \n' % argName)
    ...
    def getHostInfo(self, xmlData, xmlHeaders, index):
        ...
        if index >= 0 and index < len(self.ENUM_HOSTS):
            try:
                ...
                self.ENUM_HOSTS[index]['serverType'] = \
                    xmlHeaders.getheader('Server')
...
def msearch(argc, argv, hp):
    ...
    if argc >= 3:
        if argc == 4:
            ...
            searchType = argv[1]
            searchName = argv[2]
        st = "urn:%s:%s:%s:%s" % (st, searchType, searchName,
                                  hp.UPNP_VERSION.split('.')[0])
    else:
        st = defaultST
    ...
    #Have to create a new socket since replies will be sent directly to our IP,
    #   not the multicast IP
        ...
        if action == 'list':
            if len(hp.ENUM_HOSTS) == 0:
                print "No known hosts"\
                    " - try running the 'msearch' or 'pcap' commands"
                return
            ...
            if argc == 3:
                ...
                try:
                    ...
                    if hostInfo['dataComplete'] is True:
                        ...
                        print "Can't show host info because I don't have it."\
                            " Please run 'host get %d'" % index
            ...
            if argc == 3:
                ...
                for deviceName, deviceData in \
                        hostInfo['deviceList'].iteritems():
                    print deviceName
...
Command line usage: %s [OPTIONS]
    ...
    -i <interface>      Specify the name of the interface to use
            ...
            if opt == '-s':
                ...
                try:
                    if platform.system() != 'Windows':
                        ...
                        networkInterfaces.append('Run ipconfig to get a list '
                                                 'of available network '
                                                 'interfaces!')
                except Exception, e:
                    print 'Error opening file:', e
                    print "If you aren't running Linux,"\
...
#Main
def main(argc, argv):
    #Table of valid commands - all primary commands must
    #   have an associated function
    appCommands = {
        'help': {
            'help': None
            },
    ...
    #The load command should
    #   auto complete on the contents of the current directory
